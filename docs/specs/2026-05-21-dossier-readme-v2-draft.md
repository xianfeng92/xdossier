---
title: README v2 草稿（待替换正式 README.md）
status: draft
kind: draft-readme
owner: claude
created: 2026-05-21
updated: 2026-05-21
implements: ["docs/specs/2026-05-21-dossier-oss-launch-research.md"]
reviews: []
---

> 📝 这是 README.md 的 v2 草稿，**还没替换正式 README**。命名 placeholder 用 `<NAME>`（建议替换为 `specfold`）。GIF 占位写 `[TODO: GIF]`。
...
`<NAME>` is a **pedagogical HTML archive** for AI-generated technical docs. Reader picks a mode; the same single-file HTML adapts.
...
```bash
# Zero install:
npx <NAME>@latest render docs/specs/my-spec.md

# Or globally:
npm i -g <NAME>
<NAME> render docs/specs/my-spec.md
```
...
`<NAME>` extends both: artifact-first (not on-demand), pedagogy-first (not just visual), multi-document (not single-file).

## How does it compare?

| | `<NAME>` | html-anything | markdown-viewer/skills | Marky / MacMD |
|---|---|---|---|---|
| Tiered reader modes | ✅ | ❌ | ❌ | ❌ |
